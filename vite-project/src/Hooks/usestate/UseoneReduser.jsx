...
export default function UseoneReduser() {
  ...
  return (
    <div>
    {
      console.log(count.number,count.amount)
    }
      <h1>Count : {count.number}</h1>
      <h1>amount: {count.amount}</h1>
      <Button onClick={() => dispach({ activekey: "ind", num: 10 })}>
        inc
      </Button>
      <Button onClick={() => dispach({ activekey: "dec", num: 10 })}>
        dec
      </Button>
      <Button onClick={() => dispach({ activekey: "reset" })}>reset</Button>
      <br />
      <br /> <br />
      <Button onClick={() => dispach({ activekey: "ind_amt", num: 10 })}>
        inc-amt
      </Button>
      <Button onClick={() => dispach({ activekey: "dec_amt", num: 10 })}>
        dec-amt
      </Button>
